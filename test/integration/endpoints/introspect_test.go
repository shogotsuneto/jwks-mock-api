...
// TestIntrospection tests basic token introspection endpoint functionality
...
// TestIntrospectionScenarios tests token introspection with various user scenarios
// covering different token types and validation patterns
func TestIntrospectionScenarios(t *testing.T) {
	its := common.NewIntegrationTestSuite()
	its.WaitForAPI(t)
	
	t.Log("=== Starting Token Introspection Scenario Testing ===")
	...
	for _, scenario := range testScenarios {
		...
		if !scenario.expectExpired && !introspectResp.Active {
			t.Errorf("❌ INTROSPECTION SCENARIO FAILED: Token for %s should be active", scenario.name)
		}
		
		// Note: Scope validation removed per RFC 7662 recommendation to avoid claim content testing
	}
	
	t.Log("✅ Token Introspection Scenario Testing PASSED")
}